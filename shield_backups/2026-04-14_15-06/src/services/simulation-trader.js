...
export class SimulationTrader {
    ...
    /**
     * Machine Learning: Symbol Affinity Engine
     * Calculates how "well" the algo currently understands a specific asset.
     */
    getSymbolAffinity(symbol) {
        if (!this.tradeHistory || this.tradeHistory.length < 5) return 1.0; // Baseline
        
        const recent = this.tradeHistory.filter(t => t.symbol === symbol).slice(-20);
        if (recent.length < 3) return 1.0; // Not enough data for this specific symbol

        const symbolWins = recent.filter(t => t.profit > 0).length;
        const winRate = symbolWins / recent.length;

        // If winrate > 60%, boost confidence (Max 1.2x)
        // If winrate < 40%, dampen confidence (Min 0.7x)
        let multiplier = 1.0;
        if (winRate > 0.6) multiplier = 1.0 + ((winRate - 0.6) * 0.5); 
        if (winRate < 0.4) multiplier = 1.0 - ((0.4 - winRate) * 0.75);
    ...
    async processSignal(symbol, recommendation, currentPrice) {
        ...
        if (recommendation.action !== 'WAIT' && recommendation.isStable) {
            ...
            /* DEPRECATED: Removing sim logs from telegram
