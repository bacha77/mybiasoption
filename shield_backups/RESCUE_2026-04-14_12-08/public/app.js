...
function setupSocketListeners() {
    ...
    socket.on('price_updated', (data) => {
        ...
        // 1. GLOBAL COMPONENTS (Always live)
        if (data.watchlist) updateWatchlistUI(data.watchlist);
        if (data.boardSentiment) updateSentinelCore(data); // Sentinel handles sentiment internally
        if (data.updates) updateMarketTickerRibbon(data.updates);
        if (data.sectors) updateMarketTickerRibbon(data.sectors);
        if (data.g7Sectors) updateG7SpiderMatrix(data.g7Sectors);

        // 2. ACTIVE CHART COMPONENTS (Symbol-specific)
        if (data.symbol === currentSymbol || data.isBatch) {
            updateTerminalBoard(data);
            if (candleSeries && data.candle) {
                // Smooth real-time candle update
...
function updateTerminalBoard(data) {
    ...
    // 1. Root Level Normalization (Instant Sync)
    const price = data.currentPrice || data.price || 0;
    const score = data.confluenceScore || data.analysis?.confluenceScore || 0;
    const biasVal = data.bias || data.analysis?.bias || 'NEUTRAL';
    const narrative = data.narrative || data.analysis?.narrative || (data.analysis?.aiInsight ? data.analysis.aiInsight.slice(0, 100) + '...' : 'Syncing...');

    // 2. Global Price/Chart Sync
    if (data.symbol) {
...
function updateG7SpiderMatrix(basket) {
    ...
    currencies.forEach(curData => {
        ...
        if (whaleSyncEl) {
            const hasSmt = data.smtDivergence;
            const correl = (data.marketCorrelation || 0);
            whaleSyncEl.innerText = hasSmt ? 'SMT TRAP ACTIVE' : `${Math.round(correl * 100)}% WHALE SYNC`;
            whaleSyncEl.style.color = hasSmt ? '#ff0055' : (correl > 0.9 ? '#10b981' : 'var(--accent)');
            if (hasSmt) whaleSyncEl.classList.add('pulse');
            else whaleSyncEl.classList.remove('pulse');
        }
...
function updateSentinelCore(data) {
    ...
    // 5. Inter-Market Correlation (Whale Sync & SMT Trap)
    const hasSmt = data.smtDivergence;
    const correlVal = (data.marketCorrelation || 0);
    const isWhaleSync = Math.abs(correlVal) > 0.85;
    
    setFactor('tg-f-correl', hasSmt ? 'DANGER' : (isWhaleSync ? 'ACTIVE' : 'WARN'));
    
    const correlEl = document.getElementById('tg-correl-sync');
    if (correlEl) {
        correlEl.innerText = hasSmt ? 'MASTER TRAP ACTIVE' : `WHALE SYNC: ${Math.round(Math.abs(correlVal) * 100)}%`;
        correlEl.style.color = hasSmt ? '#ff0055' : (isWhaleSync ? 'var(--gold)' : 'var(--text-dim)');
        correlEl.style.borderColor = (hasSmt || isWhaleSync) ? 'var(--gold)' : 'rgba(255,255,255,0.05)';
        if (hasSmt) correlEl.classList.add('pulse');
        else correlEl.classList.remove('pulse');
    }
