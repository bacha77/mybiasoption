        ...
        if (now - lastWatchlistTime > 30000 || !cachedWatchlist.length) {
            ...
            cachedBoardSentiment = {
                ...
                topSignals: cachedWatchlist
                    .filter(item => (item.confluenceScore || 0) >= 70)
                    .sort((a, b) => b.confluenceScore - a.confluenceScore)
                    .slice(0, 3)
                    .map(item => ({ symbol: item.symbol, score: item.confluenceScore, bias: item.bias }))
            };
...
async function startServer() {
    ...
    simulator.onPriceUpdateCallback = (data) => {
        try {
            if (data.isBatch) {
                // 1. ENRICH BATCH WITH INSTITUTIONAL SIGNALS
                const sym = simulator.currentSymbol || 'SPY';
                data.symbol = sym; // Ensure sync for active symbol cards
                data.watchlist = processWatchlist();
                data.sectors = processSectors();
                data.g7 = calculateG7Basket();

                // 2. UNIFIED BOARD SENTIMENT PULSE
                const validSignals = data.watchlist.filter(w => w.price > 0 && w.bias && w.bias !== 'NEUTRAL');
                ...
                data.boardSentiment = {
                    ...
                    topSignals: data.watchlist
                        .filter(w => w.confluenceScore >= 75)
                ...
                // 3. RICH ACTIVE-SYMBOL UPDATE (ENSURES NO OVERWRITES)
                const activeUpdate = data.updates.find(u => u.symbol === sym);
                const richPayload = processData(sym, { 
                    basket: data.g7, 
                    sectors: data.sectors, 
                    watchlist: data.watchlist 
                });
                
                // Merge batch metadata into rich payload to keep board alive
                Object.assign(richPayload, {
                    watchlist: data.watchlist,
                    boardSentiment: data.boardSentiment,
                    isBatch: true,
                    g7Sectors: Object.entries(data.g7.basket || {}).map(([c, i]) => ({ symbol: c, perf: i.perf })),
                    equitySectors: data.sectors?.filter(s => !s.symbol?.includes('=X'))
                });

                io.emit('price_updated', richPayload);
            } else {
                // Individual Tick Process (Ultra-Rich)
                const payload = processData(data.symbol, { watchlist: processWatchlist() });
                io.emit('price_updated', payload);
            }
        } catch (err) {
            console.error("[SERVER] Emission Failure:", err);
        }
    };
 }
...
function processData(symbol, options = {}) {
    ...
    if (leaderSym && leaderSym !== symbol) {
        ...
        if (leaderCandles && leaderCandles.length > 20) {
            ...
            const leaderLast = leaderCandles[leaderCandles.length - 1];
            if (lastCandle && leaderLast) {
               const isUp = lastCandle.close > lastCandle.open;
               const leaderUp = leaderLast.close > leaderLast.open;
               // If leader is trending one way but subject is lagging/diverging = SMT Opportunity
               if (isUp !== leaderUp && marketCorrelation > 0.8) {
                   smtDivergence = true;
               }
            }
        }
    ...
    const payload = {
        ...
        confluenceScore: smtDivergence ? Math.min(100, finalConfScore + 15) : finalConfScore,
        smtDivergence,
        marketCorrelation,
        correlationLeader,
        markers: { 
    ...
    // Flattened Rich Payload
    return {
        symbol,
        currentPrice: stock.currentPrice,
        price: stock.currentPrice,
        dailyChangePercent: stock.dailyChangePercent || 0,
        dailyChangePoints: stock.dailyChangePercent ? (stock.currentPrice * (stock.dailyChangePercent / 100)) : 0,
        ...
        markers: { 
            ...
            asiaHigh: markers.asiaHigh || 0,
            asiaLow: markers.asiaLow || 0
        },
        institutionalRadar: {
            ...markers.radar,
        ...
        analysis: {
            narrative: recommendation.tacticalNarrative || bias.narrative,
            confluenceScore: finalConfScore,
            bias: bias.bias,
            alignedCount: alignedCount,
        ...
        boardSentiment: options.boardSentiment,
        candles: enrichedCandles.slice(-100),
...
function generateAIAnalystInsight(data) {
    ...
    if (data.smtDivergence) {
        insight = `🔥 HIGH CONVICTION: SMT Divergence detected against ${data.correlationLeader}. Whales are likely accumulating while retail is trapped in the opposite direction. Expect explosive displacement. ` + insight;
        prob = Math.min(99, prob + 10);
    }

    if (data.marketCorrelation > 0.9 && !data.smtDivergence) {
        insight = `🛰️ WHALE SYNC ACTIVE: Asset is moving in perfect harmony with ${data.correlationLeader} (Correlation: ${Math.round(data.marketCorrelation*100)}%). Swimming with the giants. ` + insight;
    } else if (data.marketCorrelation > 0.8) {
        insight = `🛰️ WHALE SYNC ACTIVE: Asset is moving in harmony with ${data.correlationLeader} (Correlation: ${Math.round(data.marketCorrelation*100)}%). ` + insight;
    }
