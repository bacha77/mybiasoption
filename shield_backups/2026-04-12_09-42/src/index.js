...
app.get('/api/history', (req, res) => {
    ...
    if (!stock) return res.status(404).json({ error: `Symbol ${normalizedSymbol} not loaded yet` });
...
io.on('connection', (socket) => {
    try {
        ...
        // Let the client declare what it wants to see first
        ...
        socket.on('switch_symbol', (sym) => {
            ...
            if (simulator && simulator.isInitialized) {
                const payload = {
                    ...processData(newSym),
                    history: simulator.stocks[newSym]?.candles[simulator.currentTimeframe] || [],
                    isSwitch: true
                };
...
async function runUpdateLoop() {
    try {
        ...
        const watchlist = processWatchlist();
...
function processSectors() {
    return simulator.sectors.map(s => {
        ...
        // Add "Live Jitter" to prove connection is active during off-hours
        if (price > 0) {
            const jitter = (Math.random() - 0.5) * (price * 0.00005); 
            price += jitter;
        }
