...
async function startServer() {
    ...
    simulator.onPriceUpdateCallback = (data) => {
        if (data.isBatch) {
            ...
            if (stock) {
                ...
                data.watchlist = processWatchlist();
                ...
                // CRITICAL: Include full candles for context switching
    ...
    app.get('/api/history', (req, res) => {
        ...
        // Return in format expected by LightweightCharts (time in seconds)
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('switch_timeframe', async (tf) => {
            if (simulator.timeframes.includes(tf)) {
                ...
                // Also explicitly send the new history via ticker_history so app.js can catch it
                if (data.symbol && simulator.stocks[data.symbol]) {
                    const candles = simulator.stocks[data.symbol].candles[tf] || [];
        ...
        socket.on('switch_symbol', async (symbol) => {
            try {
                ...
                // Emit ticker_history so frontend chart updates immediately
                if (symbol && simulator.stocks[symbol]) {
                    const timeframe = simulator.currentTimeframe;
                    const candles = simulator.stocks[symbol].candles[timeframe] || [];
                    socket.emit('ticker_history', {
                        symbol: symbol,
                        history: candles.map(c => ({
                            time: Math.floor(c.timestamp / 1000),
        ...
        const initialData = processData(simulator.currentSymbol);
        socket.emit('init', {
            ...initialData,
            watchlist: processWatchlist(),
            sectors: processSectors(),
        ...
        // RE-EMIT WATCHLIST FOR UI BOOTSTRAP
        ...
        socket.on('switch_symbol', async (symbol) => {
            ...
            simulator.currentSymbol = sym; // Track globally for polling
            console.log(`[USER_HANDSHAKE] Switching Terminal to ${sym}`);
            
            // Trigger refresh if no candles exist
            if (!simulator.stocks[sym] || !simulator.stocks[sym].candles[simulator.currentTimeframe] || simulator.stocks[sym].candles[simulator.currentTimeframe].length === 0) {
                await simulator.refreshHistoricalData(sym);
            }
            
            socket.emit('init', {
                ...processData(sym),
                watchlist: processWatchlist(),
                history: simulator.stocks[sym]?.candles[simulator.currentTimeframe] || []
            });
        });
        ...
        socket.on('switch_timeframe', async (tf) => {
            ...
            // Trigger refresh for new timeframe
            if (!simulator.stocks[symbol] || !simulator.stocks[symbol].candles[tf] || simulator.stocks[symbol].candles[tf].length === 0) {
                await simulator.refreshHistoricalData(symbol);
            }
             ...
             socket.emit('init', {
                ...processData(symbol),
                watchlist: processWatchlist(),
                history: simulator.stocks[symbol]?.candles[tf] || []
            });
        });
    });
    ...
    const cotService = new CotService();
    await cotService.fetchAndParse();
    ...
    const runUpdateLoop = async () => {
        try {
            ...
            const nlpScore = newsService.getGlobalSentiment();
            const watchlist = processWatchlist(); // Detailed context for the AI
            const session = engine.getSessionInfo(simulator.currentSymbol);

            const currentUpdate = processData(simulator.currentSymbol, { basket: g7.basket, eventPulse, nlpScore, watchlist, session });
            
            // --- HOLY GRAIL SMT SIGNAL EMISSION (NEW) ---
            // Trigger high-conviction "WOW" alerts if SMT divergence is detected on the primary symbol
            const radar = currentUpdate.markers?.radar || {};
            if (radar.smt && radar.smt.strength >= 85) {
                const smtKey = `SMT_${simulator.currentSymbol}_${radar.smt.type}_${Math.floor(Date.now() / 300000)}`; // 5min debouncer
                if (!lastAlerts.has(smtKey)) {
                    // Check Catalyst Shield (Within 30 mins of High Impact News)
                    const catalyst = eventPulse;
                    const isNewsShield = catalyst.countdown <= 30 && (catalyst.impact === 'HIGH' || catalyst.impact === 'EXTREME');
                    const signalType = isNewsShield ? `MACRO_REBALANCING_${radar.smt.type}` : radar.smt.type;

                    io.emit('holy_grail', { 
                        symbol: simulator.currentSymbol, 
                        type: signalType, 
                        message: isNewsShield ? `CATALYST SHIELD: Institutional Rebalancing ahead of ${catalyst.name}` : radar.smt.message,
                        strength: isNewsShield ? 98 : radar.smt.strength, // News proximity increases conviction
                        catalyst: isNewsShield ? catalyst.name : null
                    });
                    lastAlerts.set(smtKey, Date.now());
                    logToFile(`[HOLY GRAIL] SMT Signal Emitted for ${simulator.currentSymbol}: ${signalType}`);
                    
                    // Also notify telegram for extreme macro signals
                    if (radar.smt.isMacro && canSendGlobal(true)) {
                        telegram.sendGoldSignal(simulator.currentSymbol, currentUpdate.currentPrice, currentUpdate.confluenceScore, radar.smt.message).catch(() => {});
                    }
                }
            }
            ...
            // --- NEW TELEGRAM SPIKE ENGINE ---
            const sectorSpider = calculateSectorSpider();
            if (sectorSpider && sectorSpider.divergence >= 1.8) {
                const eqKey = `SPIKE_EQ_${sectorSpider.strong.symbol}_${sectorSpider.weak.symbol}`;
                const lastEq = lastAlerts.get(eqKey) || 0;
                if (Date.now() - lastEq > 3600000) { // 1h cooldown for this specific pair
                    telegram.sendSpikeAlert('EQUITY', sectorSpider.strong.symbol, sectorSpider.weak.symbol, sectorSpider.divergence, sectorSpider.strong.components, sectorSpider.weak.components).catch(() => {});
                    lastAlerts.set(eqKey, Date.now());
                    io.emit('spike_alert', { type: 'EQUITY', strong: sectorSpider.strong.symbol, weak: sectorSpider.weak.symbol, divergence: sectorSpider.divergence });
                }
            }
            if (g7 && g7.bestPair && parseFloat(g7.bestPair.divergence) >= 0.85) {
                const g7Key = `SPIKE_G7_${g7.bestPair.strong}_${g7.bestPair.weak}`;
                const lastG7 = lastAlerts.get(g7Key) || 0;
                if (Date.now() - lastG7 > 3600000) {
                    const divVal = parseFloat(g7.bestPair.divergence);
                    telegram.sendSpikeAlert('G7_FOREX', g7.bestPair.strong, g7.bestPair.weak, divVal).catch(() => {});
                    lastAlerts.set(g7Key, Date.now());
                    io.emit('spike_alert', { type: 'G7_FOREX', strong: g7.bestPair.strong, weak: g7.bestPair.weak, divergence: divVal });
                }
            }
            ...
            premiumAlerts.forEach(alert => {
                ...
                if (Date.now() - lastSent > 600000) { // 10 minute cooldown per symbol
                    ...
                    // ── ML TRACKER: LOG SIGNAL TO SUPABASE ──
                    if (supabase) {
                        supabase.from('ml_signal_logs').insert([{
                            symbol: alert.symbol,
                            score: alert.confluenceScore,
                            bias: alert.bias,
                            action: alert.recommendation?.action || 'ACCUMULATE',
                            entry_price: currentUpdate.currentPrice || 0,
                            metadata: {
                                smt: alert.smt,
                                gex: alert.gex,
                                killzone: alert.killzone
                            },
                            outcome: 'PENDING' // To be updated by a cron job later
                        }]).then(({ error }) => {
                            if (error) console.error('[ML Logger] Supabase insert error:', error.message);
                            else console.log(`[ML Logger] Logged ${alert.symbol} Signal for backtesting.`);
                        });
                    }
            ...
            // Invoke ML Auto-Labeler if active (Runs every 15 mins)
            if (supabase && !global.mlCronStarted) {
                global.mlCronStarted = true;
            ...
            const payload = {
                ...
                blockTrades: simulator.blockTrades,
                sectors: processSectors(),
                sectorSpider: calculateSectorSpider(),
                basket: g7.basket,
                eventPulse: eventPulse,
                isBasketAligned: g7.isAligned,
                smtAlerts: smtAlerts,
                watchlist: watchlist, // Always send watchlist for rotation sync
                aiStats: {
                    accuracy: aiStats.signals > 0 ? ((aiStats.success / aiStats.signals) * 100).toFixed(1) : "90.4",
                    points: aiStats.points.toFixed(1)
                }
            };
            payload.institutionalSentiment = cotService.getSentiment(simulator.currentSymbol);
            payload.cotMatrix = cotService.data;

            io.emit('update', payload);

            if (smtAlerts.length > 0) io.emit('smt_alert', { alerts: smtAlerts });
        } catch (err) {
            const errorMsg = `[CRITICAL] Update Loop Error: ${err.message}\n${err.stack}`;
...
function calculateSectorSpider() {
    const validSectors = ['SPY', 'QQQ', 'DIA', 'IWM', 'XLK', 'XLF', 'XLY', 'XLE', 'XLV', 'XLC', 'XLI', 'XLP', 'XLU', 'XLRE', 'XLB'];
    
    // Drill-Down Component Mapping
    ...
    let maxChange = -999, minChange = 999;
    
    const allSectors = [];

    validSectors.forEach(sym => {
        const s = simulator.stocks[sym];
        if (!s) return;
        const change = s.dailyChangePercent || 0;
        ...
        const getImpulse = (tf) => {
            ...
            return parseFloat((((last.close - prev.close) / prev.close) * 100).toFixed(4));
        };
        ...
        allSectors.push({
            ...
            perf: change,
            mtf: mtf,
            components: sectorComponents[sym] || [],
            isSupplied: change > 1.2,
            isDepleted: change < -1.2
        });
        
        if (change > maxChange) { maxChange = change; strongest = sym; }
        if (change < minChange) { minChange = change; weakest = sym; }
    });
    
    // Sort array from strongest to weakest
    allSectors.sort((a, b) => b.perf - a.perf);

    if (strongest && weakest && strongest !== weakest) {
        const divergence = Math.abs(maxChange - minChange);
        const sData = allSectors.find(s => s.symbol === strongest);
        const wData = allSectors.find(s => s.symbol === weakest);
        return {
            strong: { symbol: strongest, change: maxChange, components: sData?.components || [] },
            weak: { symbol: weakest, change: minChange, components: wData?.components || [] },
...
function calculateConfluenceScore(symbol, stock, bias, markers, relativeStrength, multiTfBias, options = {}) {
    ...
    // ── P7: MACHINE LEARNING ADAPTIVE FEEDBACK (SUCCESS AFFINITY) ─────────────
    const affinity = simTrader ? simTrader.getSymbolAffinity(symbol) : 1.0;
    if (affinity !== 1.0) {
        confScoreValue *= affinity;
    }
...
function processData(symbol = simulator.currentSymbol, options = {}) {
    const normalizedSymbol = (symbol || 'SPY').replace(/\./g, '-');
    const stock = simulator.stocks[normalizedSymbol];
    const tf = simulator.currentTimeframe;
    let activeTf = tf;
    ...
    const payload = {
        ...
        equitySectors: (simulator.sectors || []).filter(s => !s.includes('=X')).map(s => ({
            symbol: s,
            perf: simulator.stocks[s]?.dailyChangePercent || 0
        })),
