...
export class RealDataManager {
    constructor() {
        ...
        this.sectors = ['SPY', 'QQQ', 'DIA', 'IWM', 'BTC-USD', 'GLD', 'XLK', 'XLY', 'XLF', 'XLC', 'XLE', 'XLV', 'XLI', 'XLP', 'XLU', 'XLRE', 'XLB', 'SMH', 'NVDA', 'AMD', 'META', 'GOOGL', 'TSLA', 'AAPL', 'MSFT', 'KRE', 'XBI', 'IYT', 'EURUSD=X', 'GBPUSD=X', 'USDJPY=X', 'AUDUSD=X', 'NZDUSD=X', 'USDCAD=X', 'USDCHF=X', '^TNX', 'UUP'];
        this.symbolMap = {
        ...
        if (quote.prevClose && quote.prevClose > 0) {
            ...
            if (dp !== 0 && stock.currentPrice > 0) {
                ...
                const stableBaselines = { 
                    ...
                    'DXY': 104.00, 'DX-Y': 104.00, 'DX-Y.NYB': 104.00,
                    '^VIX': 18.50, 'VIX': 18.50, 'BTC-USD': 66400.00, 'GC=F': 2550.50
                };
        ...
        try {
            ...
            try {
                const dailyRes = await Promise.race([
                    yahooFinance.chart(symbol, { period1: p1String, interval: '1d' }, { validate: false }),
    ...
    async updateAll() {
        ...
        const now = Date.now();
        // Quote update is fast, do it every 15s - Batching allows higher frequency
        if (!this.lastQuoteUpdate || now - this.lastQuoteUpdate > 15000) {
            // Batch all sectors + macros together for massive speedup
            const batchSymbols = ['^VIX', 'DX-Y.NYB', ...this.sectors];
    ...
    getInstitutionalMarkers(symbol = this.currentSymbol, tf = this.currentTimeframe) {
        ...
        const radar = this.getInstitutionalRadar(symbol, tf);
