...
window._lastSpokenAlert = "";
window._lastSpeakTime = 0;
...
function setupSocketListeners() {
    ...
    socket.on('update', (data) => {
        ...
        // 2. Main Board Logic: Extract current symbol from batch if needed
        let targetData = data;
        if (data.isBatch && data.updates) {
            const active = data.updates.find(u => u.symbol === currentSymbol);
            if (active) targetData = { ...data, ...active };
        }

        if (targetData.symbol === currentSymbol || targetData.isBatch || (!targetData.symbol && targetData.currentPrice)) {
            updateTerminalBoard(targetData);
        }
    });
...
function updateTerminalBoard(data) {
    ...
    try {
        ...
        // 10. Tier 1 Institutional Edge Panel
        if (data.markers) {
        ...
        console.error('[BOARD ERROR] Critical failure in render cycle:', err);
    }
}
...
function speakText(text) {
    if (!window._voiceEnabled || !window.speechSynthesis) return;
    const now = Date.now();
    if (text === window._lastSpokenAlert && now - window._lastSpeakTime < 60000) return; // Prevent spam
    
    window.speechSynthesis.cancel(); // Interrupt previous
    const utter = new SpeechSynthesisUtterance(text);
    utter.rate = 1.05;
    utter.pitch = 0.95; // More professional/bass
    window.speechSynthesis.speak(utter);
    window._lastSpokenAlert = text;
    window._lastSpeakTime = now;
}

function initSentinelListeners() {
    const vBtn = document.getElementById('tg-voice-toggle');
    if (vBtn) {
        vBtn.onclick = () => {
            window._voiceEnabled = !window._voiceEnabled;
            vBtn.innerText = `VOICE: ${window._voiceEnabled ? 'ON' : 'OFF'}`;
            vBtn.style.background = window._voiceEnabled ? 'rgba(16,185,129,0.1)' : 'rgba(255,157,0,0.1)';
            vBtn.style.color = window._voiceEnabled ? 'var(--bullish)' : 'var(--gold)';
            if (window._voiceEnabled) speakText("Vocal sync activated.");
        };
    }
}
...
function updateSentinelCore(data) {
    ...
    if (data.boardSentiment) {
        ...
        if (signalsEl && bs.topSignals) {
            if (bs.topSignals.length > 0) {
                let signalHtml = 'TOP RADAR SIGNALS:<br>';
                bs.topSignals.forEach(sig => {
                    const color = sig.bias.includes('BULLISH') ? 'var(--bullish)' : 'var(--bearish)';
                    signalHtml += `<span style="color:${color}">${sig.symbol}</span>: ${sig.score}% ${sig.bias} <br>`;
                });
                signalsEl.innerHTML = signalHtml;
            } else {
                signalsEl.innerHTML = 'MONITORING WATCHLIST... NO HIGH-CONVICTION SIGNALS DETECTED.';
            }
        }
    }
    ...
    // --- NEURAL PULSE & VOICE ENGINE ---
    const cardEl = document.getElementById('trade-guardian-card');
    if (cardEl) {
        if (absConviction > 80) cardEl.classList.add('neural-pulse-high');
        else { cardEl.classList.remove('neural-pulse-high'); }
    }

    // Speak High-Conviction Board Alerts
    if (data.boardSentiment && data.boardSentiment.topSignals) {
        const best = data.boardSentiment.topSignals[0];
        if (best && best.score >= 90) {
            speakText(`${best.symbol} ALGO ALERT: ${best.score}% CONVICTION ${best.bias}.`);
        }
    }
}
