...
// --- 🛡️ ENGINE STABILITY MODE ACTIVE ---
    ...
    try {
        ...
        socket.on('join_symbol', (data) => {
            ...
            if (simulator) {
                ...
                const payload = {
                    ...processData(sym, { spiders }),
                    sectors: processSectors(),
                    watchlist: processWatchlist(),
                    isBatch: true,
                    symbol: sym,
                    currentPrice: simulator.stocks[sym]?.currentPrice || 0
                };
                socket.emit('init', payload);
                console.log(`[SOCKET] Instant-Flow payload delivered for ${sym}`);
            }
        });
    ...
    simulator.onPriceUpdateCallback = (data) => {
        if (data.isBatch) {
            ...
            if (stock) {
                ...
                // CRITICAL: Ensure symbol and price are set for frontend sync
                data.symbol = symbol;
                data.price = stock.currentPrice;
                data.currentPrice = stock.currentPrice;
                data.dailyChangePercent = stock.dailyChangePercent;
                data.dailyChangePoints = (stock.currentPrice - stock.previousClose) || 0;
                ...
                // 2. Sector Matrices
                const spiders = calculateSectorSpider();
                data.equitySectors = spiders.all || [];
                ...
                const currentUpdateForAnalyst = processData(symbol, { spiders, session: engine.getSessionInfo(symbol) });
                data.analysis = {
                    confluenceScore: currentUpdateForAnalyst.confluenceScore,
                    amdPhase: markers.radar?.amdPhase || 'ACCUMULATION',
                    narrative: currentUpdateForAnalyst.aiInsight?.text
                };
            ...
            // Diagnostic Sync: Monitor emittance for Zero-State detection
            if (data.symbol === simulator.currentSymbol) {
                logToFile(`[DATA] Emitted ${data.symbol}: $${data.currentPrice} (${data.dailyChangePercent ? data.dailyChangePercent.toFixed(2) : 0}%) | Confluence: ${data.analysis?.confluenceScore}%`);
            }
...
function processData(symbol, options = {}) {
    ...
    const payload = {
        ...
        aiInsight: generateAIAnalystInsight({
            ...
            // Correlation Mapping
            topSector: options.spiders?.strong?.symbol,
            isCorrelated: options.spiders?.strong ? (options.spiders.strong.relativeStrength > 0.5 && bias.bias.includes('BULLISH')) : false
        }),
...
function generateAIAnalystInsight(data) {
    ...
    if (data.topSector) {
        if (data.isCorrelated) {
            insight += `SYSTEMIC SYNC: ${symbol} is perfectly aligned with leader ${data.topSector}. Institutional conviction is MAXIMUM. `;
            prob += 10;
        } else if (bias.includes('BULLISH') && data.topSector === symbol) {
             insight += `LEADERSHIP: ${symbol} is currently the MARKET LEADER (Relative Strength). High probability of trend continuation. `;
             prob += 5;
        }
    }
