...
export class InstitutionalAlgorithm {
    ...
    /**
     * Calculated VWAP Deviation Bands (Standard Deviation)
     * Institutions use these as "Overbought/Oversold" targets.
     */
    calculateVWAPBands(price, vwap, candles) {
        if (!vwap || candles.length < 20) return { upper1: 0, upper2: 0, lower1: 0, lower2: 0, zone: 'NEUTRAL' };
        
        const variance = candles.slice(-20).reduce((sum, c) => sum + Math.pow(c.close - vwap, 2), 0) / 20;
        const stdDev = Math.sqrt(variance);
        
        const u1 = vwap + stdDev;
        const u2 = vwap + (stdDev * 2);
        const l1 = vwap - stdDev;
        const l2 = vwap - (stdDev * 2);
        
        let zone = 'VALUE AREA';
        if (price > u2) zone = 'EXTREME PREMIUM';
        else if (price > u1) zone = 'PREMIUM';
        else if (price < l2) zone = 'EXTREME DISCOUNT';
        else if (price < l1) zone = 'DISCOUNT';
        
        return { upper1: u1, upper2: u2, lower1: l1, lower2: l2, zone };
    }

    /**
     * Opening Range Breakout (ORB)
     * The first 30 minutes of the NY session.
     */
    calculateORB(candles) {
        const nyNow = new Date(new Date().toLocaleString("en-US", { timeZone: "America/New_York" }));
        const orbStart = new Date(nyNow);
        orbStart.setHours(9, 30, 0, 0);
        const orbEnd = new Date(nyNow);
        orbEnd.setHours(10, 0, 0, 0);

        const orbCandles = candles.filter(c => {
            const d = new Date(c.timestamp);
            return d >= orbStart && d <= orbEnd;
        });

        if (orbCandles.length === 0) return { label: 'AWAITING NY OPEN', orbHigh: 0, orbLow: 0, breakout: 'NONE' };
        ...
        let breakout = 'NONE';
        if (current > high) breakout = 'BULLISH';
        else if (current < low) breakout = 'BEARISH';

        return {
            label: `ORB ${breakout !== 'NONE' ? 'BREAKOUT' : 'SCANNING'}`,
            orbHigh: high,
            orbLow: low,
            breakout
        };
     ...
     * Current volume relative to the average of the last 20 periods.
     */
    calculateRVOL(candles) {
        if (candles.length < 21) return { rvol: 1.0, label: 'NORMAL' };
        ...
        let label = 'NORMAL';
        if (ratio > 2.0) label = 'EXPLOSIVE';
        else if (ratio > 1.5) label = 'HIGH';
        else if (ratio < 0.5) label = 'THIN';

        return { rvol: ratio, label };
    }

    /**
     * Volume Point of Control (VPOC)
     * The price level with the highest volume in the current lookback.
     */
    calculateVPOC(candles) {
        if (candles.length === 0) return { vpoc: 0, vah: 0, val: 0, currentZone: 'NEUTRAL' };
        
        const priceMap = {};
        const step = candles[0].close * 0.0005; // 0.05% resolution
        
        candles.forEach(c => {
            const level = Math.round(c.close / step) * step;
            priceMap[level] = (priceMap[level] || 0) + c.volume;
        });

        let vpoc = 0;
        let maxVol = 0;
        Object.entries(priceMap).forEach(([price, vol]) => {
            if (vol > maxVol) {
                maxVol = vol;
                vpoc = parseFloat(price);
            }
        });

        // Simplified Value Area (VAH/VAL at +/- 1 ATR)
        const current = candles.length > 0 ? candles[candles.length - 1].close : 0;
        const lastCandle = candles[candles.length - 1];
        const atr = lastCandle ? (lastCandle.high - lastCandle.low) * 5 : 0;
        const vah = vpoc + atr;
        const val = vpoc - atr;

        let zone = 'VALUE AREA';
        if (current > vah) zone = 'PREMIUM';
        else if (current < val) zone = 'DISCOUNT';

        return { vpoc, vah, val, currentZone: zone };
    }

    /**
     * Equal Highs & Equal Lows (Stop pools)
     */
    calculateEqualLevels(candles) {
        if (candles.length < 50) return { equalHighs: [], equalLows: [] };
        
        const highCounts = {};
        const lowCounts = {};
        const thresh = candles[0].close * 0.0002; 
        
        candles.slice(-50).forEach(c => {
            const h = Math.round(c.high / thresh) * thresh;
            const l = Math.round(c.low / thresh) * thresh;
            highCounts[h] = (highCounts[h] || 0) + 1;
            lowCounts[l] = (lowCounts[l] || 0) + 1;
        });

        const equalHighs = Object.entries(highCounts)
            .filter(([p, count]) => count >= 2)
            .map(([p, count]) => ({ level: parseFloat(p), count }))
            .sort((a, b) => b.count - a.count);

        const equalLows = Object.entries(lowCounts)
            .filter(([p, count]) => count >= 2)
            .map(([p, count]) => ({ level: parseFloat(p), count }))
            .sort((a, b) => b.count - a.count);
