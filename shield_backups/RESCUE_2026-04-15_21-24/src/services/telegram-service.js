...
[View IR-Reality Card](http://localhost:3000)
    ...
    async sendSpikeAlert(type, strong, weak, divergence, strongComps = [], weakComps = []) {
        const emoji = type === 'EQUITY' ? '🏢⚡️' : '🌍💱';
        const title = type === 'EQUITY' ? 'SECTOR ROTATION SPIKE' : 'G7 CURRENCY SPIKE';
        const spreadLabel = type === 'EQUITY' ? 'Net Flow Spread' : 'Basket Divergence';
        
        const buyAction = type === 'EQUITY' ? 'BUY (Top Heavyweights)' : 'BUY (Strongest)';
        const sellAction = type === 'EQUITY' ? 'SELL (Bottom Heavyweights)' : 'SELL (Weakest)';

        let message = `
${emoji} *${title}*
----------------------------
*Status:* HIGH DIVERGENCE DETECTED
*${spreadLabel}:* ${divergence.toFixed(2)}%

🟢 *${buyAction}:* ${strong}
${strongComps.length > 0 ? `_↳ ${strongComps.join(', ')}_` : ''}

🔴 *${sellAction}:* ${weak}
${weakComps.length > 0 ? `_↳ ${weakComps.join(', ')}_` : ''}

_Institutional capital is rotating aggressively. High-probability straddle pair identified._

[View Terminal](http://localhost:3000)
        `.trim();
